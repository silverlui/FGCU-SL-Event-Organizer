...
class EventBrowser(QWidget):
    ...
    def scrape_agencies(self):
        try:
            ...
            self.build_faiss_index_for_agencies()
    ...
    def build_faiss_index_for_agencies(self):
        agency_names = [agency["Name"] for agency in self.agencies]
        embeddings = self.model.encode(agency_names, convert_to_tensor=False).astype("float32")
        self.suggestion_index = faiss.IndexFlatL2(embeddings.shape[1])
        self.suggestion_index.add(embeddings)
